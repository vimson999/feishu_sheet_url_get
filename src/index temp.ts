  ...
  execute: async (formItemParams, context) => {
    ...
    function debugLog(arg: any) {
      ...
      if (res.code === 200 && res.data) {
        ...
        //使用task_id作为参数 get 请求 /extract/status/{task_id} 地址获取最终数据
        const final_url = `http://127.0.0.1:8083/api/media/extract/status/${task_id}`;
        // const final_url = `http://121.4.126.31/api/media/extract/status/${task_id}`;
        const final_response = await context.fetch(final_url, {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
            'x-source': 'feishu-sheet',
        ...
        if (final_response.code === 200 && final_response.data) {
          const mediaData = final_response.data;
